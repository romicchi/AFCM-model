...
def remove_stops(text):
    ...
    # Remove stopwords, lemmatize and stem the words
...
# Define the pipeline with TF-IDF vectorizer, feature selection, TruncatedSVD and SVM classifier
pipeline = Pipeline([
    ('vectorizer', TfidfVectorizer(lowercase=True,
                                   max_features=50,  # Reduce the number of features
                                   max_df=0.5,  
                                   min_df=10,  
                                   ngram_range=(1, 3),
                                   stop_words="english"
    ...
    ('svd', TruncatedSVD(n_components=30)),  # Reduce the number of components
...
joblib.dump(pipeline, pipeline_output_path, compress=4)
